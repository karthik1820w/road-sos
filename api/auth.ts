...
// Lazily created so importing this module (e.g. in tests, or when auth is unused) never crashes the process.
...
const getSupabase = () => {
  if (!_supabase) {
    ...
    _supabase = createClient<any, any, any>(url, key);
  }
  return _supabase;
};
const supabase: SupabaseClient<any, any, any> = new Proxy({} as SupabaseClient<any, any, any>, { get: (_t, prop) => (getSupabase() as any)[prop] });
...
router.post('/login', loginLimiter, async (req, res) => {
  try {
    ...
    const token = jwt.sign({ id: user.id, email: safeEmail }, getJwtSecret(), { expiresIn: JWT_EXPIRES_IN });

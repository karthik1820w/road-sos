...
import { normalizePhone } from '../services/incidentService';
...
export const PermissionsModal: React.FC<PermissionsModalProps> = ({ onComplete }) => {
  ...
  const requestLocation = async () => {
    try {
      ...
      // Record the truth; the app shows "protection limited" instead of pretending GPS works.
      setLocGranted(false);
      localStorage.setItem('roadsos_perm_location', 'denied');
      setStep(2);
    }
  };
  ...
  const requestMic = () => {
    navigator.mediaDevices.getUserMedia({ audio: { echoCancellation: true, noiseSuppression: true, autoGainControl: true } })
      ...
      .catch((err) => {
        ...
        setMicGranted(false);
        localStorage.setItem('roadsos_perm_mic', 'denied');
        setStep(3);
      });
  };
  ...
  const finishSetup = () => {
    ...
    localStorage.setItem('roadSosUserPhone', normalized);
    localStorage.setItem('roadsos_perm_location', locGranted ? 'granted' : 'denied');
    localStorage.setItem('roadsos_perm_mic', micGranted ? 'granted' : 'denied');

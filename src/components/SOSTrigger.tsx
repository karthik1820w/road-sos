...
export const SOSTrigger: React.FC<SOSTriggerProps> = ({ onTrigger, isPulsing = false }) => {
  ...
  // Keep the latest onTrigger without restarting the countdown when the parent re-renders.
      ...
      countdownTimerRef.current = setInterval(() => {
        setCountdown(prev => {
          if (prev <= 1) {
            ...
            // Reset so the button is usable again and never shows a stale "0s" countdown.
            setTimeout(() => { setPressState('IDLE'); setCountdown(5); setHoldProgress(0); }, 0);
            return 0;
          }
          return prev - 1;
        });
  ...
  // Unmount only: never leave the 50 ms hold interval running (it could fire a false SOS).
